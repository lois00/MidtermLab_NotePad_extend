...
public class NoteEditor extends Activity {
    ...
    /*
     * Creates a projection that returns the note ID,the note title,the note contents,
     * the note created date and the background color of note.
     */
    private static final String[] PROJECTION =
            new String[]{
                    ...
                    NotePad.Notes.COLUMN_NAME_CREATE_DATE,     // Projection position 3, the note's created date
                    NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR // Projection position 4, the note's modified date
            };
...
//                canvas.drawLine(r.left, baseline + 1, r.right, baseline + 1, paint);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        while (mCursor.moveToNext()) {
            int colorIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR);
            //sets the background color of the note content
            tvNote.setBackgroundResource(mCursor.getInt(colorIndex));//设置note内容的背景色
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mState == STATE_EDIT) {
            ...
            //只有当前note是已保存的note时，才添加额外菜单项Edit title和Export到选项菜单列表中，
            //因为TitleEditor和NoteExport的intent filter里都包含CATEGORY_ALTERNATIVE类别。
            /**
             * Only when current note has been saved,append to the
             * menu items for any other activities that can do stuff with it
             * as well.  This does a query on the system for any activities that
             * implement the ALTERNATIVE_ACTION for our data, adding a menu item
             * for each one that is found.
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (savedNote.equals(currentNote)) {//如果当前Note和游标所指的note的内容相同，即没有改变
            menu.findItem(R.id.menu_revert).setVisible(false);//则不显示返回撤销菜单
        } else {
            menu.findItem(R.id.menu_revert).setVisible(true);//否则显示撤销菜单
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_change_color:
                /**
                 * 当用户点击背景色菜单时，启动NoteChangeColor activity。
                 * when the menu Background clicked start the NoteChangeColor activity.
                 */
                startActivityForResult(new Intent("android.intent.action.EDIT_BACKGROUND"), 0);
                break;
        }
    ...
    /**
     * The method is called when NoteChangeColor activity calls the "setResult" method.
     * 当NoteChangeColor activity调用setResult方法时回调。
     *
...
//END_INCLUDE(paste)
    ...
    private final void updateNote(String text, String title) {
        ...
        SimpleDateFormat format = new SimpleDateFormat(NotePad.Notes.DATETIME_FORMAT);
